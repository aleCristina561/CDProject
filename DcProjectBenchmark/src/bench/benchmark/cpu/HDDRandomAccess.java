    ...
    class RandomAccess {
        ...
        public long randomReadFixedSize(String filePath, int bufferSize,
            ...
            while (counter++ < toRead) {
                ...
                long position = Math.abs(rand.nextLong()) % (fileSize - bufferSize + 1);
                file.seek(position); // Move to random position
        ...
        public int randomReadFixedTime(String filePath, int bufferSize,
            ...
            long durationNano = millis * 1_000_000L;
            // read for a fixed amount of time
            while ((System.nanoTime() - startTime) < durationNano) {
                long position = Math.abs(rand.nextLong()) % (fileSize - bufferSize + 1);
                file.seek(position);
                file.readFully(bytes);
                counter++;
